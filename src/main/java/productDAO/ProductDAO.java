...
//    }
    ...
    public Product selectProduct(int id) {
        ...
        String sql = "SELECT p.ProductID, p.ProductName, p.Description, p.Price, p.CreatedDate, p.CompanyID, pi.ImageURL "
                + "FROM PRODUCTS p "
        ...
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                // Tạo đối tượng Product với thông tin lấy được từ cơ sở dữ liệu
                product = new Product(productId, productName, description, price, createdDate, companyID);

                // Nếu có hình ảnh, thêm hình ảnh vào sản phẩm
                if (imageUrl != null) {
                    product.addImage(new ProductImage(productId, imageUrl));
                }
            }
            con.close();
        } catch (SQLException ex) {
...
//            }
            while (rs.next()) {
                ...
                int companyID = rs.getInt("CompanyID");
                String imageUrl = rs.getString("ImageURL");
                Product product = new Product(productId, productName, description, price, createdDate, companyID);
